...
const ParkingDetailsScreen = ({ route }) => {
  ...
  const handleSubmitAvailability = async () => {
    ...
    try {
      const apiUrl = `http://192.168.68.104:4001/addOrUpdateParkingAvailability?locationId=${selectedLocation.id}&available=${newAvailability}`;
  ...
  const fetchAvailability = async () => {
    try {
      const apiUrl = `http://192.168.68.104:4001/parking-availability?locationId=${selectedLocation.id}`;
      ...
      if (!response.ok) {
        console.error("Error fetching parking availability");
        return;
      }
  ...
  const handleGetDirections = async () => {
    ...
    try {
      ...
      const apiUrl = `http://192.168.68.104:3002/getDirections?startLat=${userLocation.latitude}&startLong=${userLocation.longitude}&endLat=${selectedLocation.lat}&endLong=${selectedLocation.long}`;
      const response = await fetch(apiUrl, {
        method: "GET",
      });
